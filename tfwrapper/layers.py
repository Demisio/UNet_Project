...
def upconv2d_layer(X, name, num_filters=32, kernel_size=(4, 4), strides=(2, 2), padding="SAME"):
    ...
    :param strides
        Stride information
    from https://github.com/ghwatson/faststyle/blob/master/im_transf_net.py
    """
    ...
    # down one stride.
    new_h = X_shape[1] * strides[0] ** 2
    new_w = X_shape[2] * strides[1] ** 2
    upsized = tf.image.resize_images(X, [new_h, new_w], method=1)

    # Now convolve to get the channels to what we want.
    weight_shape = [kernel_size[0], kernel_size[1], n_ch_in, num_filters]
    with tf.variable_scope(name):
...
def upconv3d_layer(X, name, num_filters=32, kernel_size=(4, 4, 4), strides=(2, 2, 2), padding="SAME"):
    """Resizes then applies a convolution.
    :param X
        Input tensor
    :param num_filters
    ...
    new_h = X_shape[1] * strides[0] ** 2
    new_w = X_shape[2] * strides[1] ** 2
    new_z = X_shape[3] * strides[2] ** 2

    # don't use tensorflow image resize!!!
    # https://hackernoon.com/how-tensorflows-tf-image-resize-stole-60-days-of-my-life-aba5eb093f35
    # https://github.com/tensorflow/tensorflow/issues/6720
    # method=1 is nearest neighbour
    # method=0 is bilinear
    # method=0, align_corners=True
    # NEED 3D method!!!

    # per feature, first in h,w
    Xreshaped = tf.transpose(X, perm=(4, 1, 2, 3, 0))

    tmp = tf.image.resize_images(Xreshaped[:, :, :, :, 0], [new_h, new_w], method=1)
    tmp2 = tf.transpose(tmp, perm=(0, 1, 3, 2))
    # reshape in z
    upsizedReshaped = tf.expand_dims(tf.image.resize_images(tmp2, [new_h, new_z], method=1), -1)
    for b in range(1, n_b_in):
        tmp = tf.image.resize_images(Xreshaped[:, :, :, :, b], [new_h, new_w], method=1)
        tmp2 = tf.transpose(tmp, perm=(0, 1, 3, 2))
        tmp3 = tf.expand_dims(tf.image.resize_images(tmp2, [new_h, new_z], method=1), -1)
        upsizedReshaped = tf.concat((upsizedReshaped, tmp3), -1)

    upsized = tf.transpose(upsizedReshaped, perm=(4, 1, 3, 2, 0))
    del upsizedReshaped

    # Now convolve to get the channels to what we want.
    weight_shape = [kernel_size[0], kernel_size[1], kernel_size[2], n_ch_in, num_filters]
    with tf.variable_scope(name):
        W = tf.get_variable(name='W',
                            shape=weight_shape,
                            dtype=tf.float32,
...
def upconv2D_layer_bn(bottom,
                      ...
                      weight_init='he_normal'):
    '''
    Shortcut for batch normalised 2D upconvolutional layer
    '''
    upco = upconv2d_layer(X=bottom,
                          name=name,
                          num_filters=num_filters,
                          kernel_size=kernel_size,
                          strides=strides,
    ...
    upco_bn = batch_normalisation_layer(upco, name + '_bn', training=training)

    act = activation(upco_bn)

    return act


def upconv3D_layer_bn(bottom,
                      name,
                      training,
                      kernel_size=(4, 4, 4),
                      num_filters=32,
                      ...
                      weight_init='he_normal'):
    '''
    Shortcut for batch normalised 3D upconvolutional layer
    '''
    upco = upconv3d_layer(X=bottom,
                          name=name,
                          num_filters=num_filters,
                          kernel_size=kernel_size,
                          strides=strides,
